...
create type SchemaTuple as ("table" text, "size" text, "attributes" text);
...
begin
	for rec in
		select r.relname, a.attname, t.typname, a.attlen, a.atttypmod
		from   pg_class r
	...
	loop
		...
		if (rec.atttypmod < 0) then
			size := size + rec.attlen;
		else
		end if;
		if (attrs <> '') then
			attrs := attrs || ', ';
